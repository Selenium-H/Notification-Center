...
const NotificationCenter = new Lang.Class({
  ...
  {
    if (this.prefs.get_boolean(KEY) == false) {
      return;
    }
  ...
  manageAutohide: function()
  {
    ...
    if (this.prefs.get_boolean("show-events")) {
      if (Config.PACKAGE_VERSION < "3.32.0") {
        if (this._messageList._eventsSection._canClear() == false) {
          this.actor.hide();
        }
        else {
          this.actor.show();
  ...
  {
    if (this.prefs.get_boolean("show-events") == false) {
      return;
    }
